...
function coverPosition(index,total){return index===0?'0%':(index/(total-1)*100)+'%';}
function staticPortfolioCard(p,total){var tags=(p.tags||[]).join(' ');return '<a class="portfolio-card" href="portfolio.html#'+encodeURIComponent(p.slug)+'"><div class="portfolio-photo" style="background-image:var(--portfolio-cover-sprite);background-size:800% 100%;background-position:'+coverPosition(p.coverIndex,total)+' 0"></div><div class="portfolio-body"><div class="portfolio-title">'+esc(p.title)+'</div><div class="portfolio-kind">'+esc(p.kind)+'</div><div class="portfolio-tags">'+esc(tags)+'</div><span class="card-arrow">→</span></div></a>';}
function renderStaticPortfolio(){var api=window.DAHAM_PORTFOLIO_STATIC;if(!portfolioGrid||!api)return false;var rows=api.listProjects();if(!rows.length)return false;portfolioGrid.innerHTML=rows.slice(0,4).map(function(row){return staticPortfolioCard(row,rows.length);}).join('');if(window.DAHAM_PORTFOLIO_IMAGES)window.DAHAM_PORTFOLIO_IMAGES.applyCoverSprite().catch(function(){});return true;}
...
bindInstagram();
if(!renderStaticPortfolio())loadPortfolio();
})();
