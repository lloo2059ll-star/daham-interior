...
var KAKAO_CHANNEL_URL='https://pf.kakao.com/_xnvxgTX';
...
var sourceChannel=window.DAHAM_WEBSITE_PUBLIC?window.DAHAM_WEBSITE_PUBLIC.detectSourceChannel(location.href,document.referrer):'website';
...
var SEARCHABLE_PROJECTS=['prugio-castle-a-32','imeun-kolon-35','bonggok-hyunjin-36','okgye-epyeon-35'];
function projectHref(slug){return SEARCHABLE_PROJECTS.indexOf(slug)>=0?'portfolio/'+encodeURIComponent(slug)+'.html':'portfolio.html#'+encodeURIComponent(slug);}
function staticPortfolioCard(p){var tags=(p.tags||[]).join(' ');return '<a class="portfolio-card" href="'+projectHref(p.slug)+'"><div class="portfolio-photo"><img src="'+esc(p.thumbnailImage||p.coverImage)+'" alt="'+esc(p.title)+'" loading="lazy" style="object-position:'+esc(p.coverPosition||'center center')+'"></div><div class="portfolio-body"><div class="portfolio-title">'+esc(p.title)+'</div><div class="portfolio-kind">'+esc(p.kind)+'</div><div class="portfolio-tags">'+esc(tags)+'</div><span class="card-arrow">→</span></div></a>';}
function renderStaticPortfolio(){var api=window.DAHAM_PORTFOLIO_STATIC;if(!portfolioGrid||!api)return false;var rows=api.listProjects();if(!rows.length)return false;portfolioGrid.innerHTML=rows.slice(0,4).map(staticPortfolioCard).join('');return true;}
function portfolioCard(p){var meta=[p.location,p.areaPyeong?p.areaPyeong+'평':''].filter(Boolean).join(' ');var tags=[p.style?'#'+p.style.replace(/\s+/g,''):'','#DAHAM'].filter(Boolean).join(' ');var staticApi=window.DAHAM_PORTFOLIO_STATIC;var cardImage=staticApi?staticApi.cardImageForProject(p.slug,p.coverImageUrl):p.coverImageUrl;var image=cardImage?'<img src="'+esc(cardImage)+'" alt="'+esc(p.title)+'" loading="lazy">':'';return '<a class="portfolio-card" href="'+projectHref(p.slug)+'"><div class="portfolio-photo">'+image+'</div><div class="portfolio-body"><div class="portfolio-title">'+esc(p.title||'DAHAM PROJECT')+'</div><div class="portfolio-kind">'+esc(meta||'전체 인테리어')+'</div><div class="portfolio-tags">'+esc(tags)+'</div><span class="card-arrow">→</span></div></a>';}
...
function fetchPortfolio(fields){return sb.from('website_portfolio').select(fields).eq('is_published',true).order('sort_order',{ascending:true}).limit(4);}
function loadPortfolio(){if(!sb)return;fetchPortfolio(PORTFOLIO_FIELDS).then(function(res){if(res.error&&isMissingGalleryColumn(res.error))return fetchPortfolio(PORTFOLIO_LEGACY_FIELDS);return res;}).then(function(res){if(res.error)throw res.error;renderPortfolio(res.data||[]);}).catch(function(){});}
...
function bindKakao(){
  document.querySelectorAll('.kakao-btn').forEach(function(link){
    link.removeAttribute('data-open-inquiry');link.href=KAKAO_CHANNEL_URL;link.target='_blank';link.rel='noopener noreferrer';
  });
}
