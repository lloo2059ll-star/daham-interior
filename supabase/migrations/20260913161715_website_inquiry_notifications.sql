...
begin
  ...
  if new.source_channel in ('website', 'naver_blog') then
    ...
    if v_company_id is not null and v_actor_id is not null then
      ...
      values (v_company_id, v_actor_id, null, 'consultation', v_consult_id, 'create', '신규 상담 · ' || v_source_label, v_consult_title || '가 접수되었습니다.', '{}'::jsonb, 'consult.html?consult=' || v_consult_id, v_dedupe_key)
      returning id into v_event_id;

      insert into public.notification_outbox(company_id, event_id, kind, title, body, target_url, dedupe_key)
      values (v_company_id, v_event_id, 'activity', '신규 상담 · ' || v_source_label, v_notification_body, 'consult.html?consult=' || v_consult_id, v_dedupe_key)
