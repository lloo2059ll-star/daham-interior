...
var spaces={
 ...
 'geochang-prugio-34':[['공사 방향','기존 구조를 확인한 뒤 주방 동선을 먼저 정리하고, 공사 중 배관과 수납 위치를 조정했습니다.'],['주방','거실보다 먼저 주방을 설계해 조리·수납·식사 동선이 겹치지 않도록 구성했습니다.'],['거실','주방과 시선이 자연스럽게 이어지도록 벽면과 조명을 정리해 넓어 보이는 중심 공간을 만들었습니다.'],['욕실','기존 설비 위치를 점검하고 타일·수납·욕실 동선을 실사용 기준으로 다시 맞췄습니다.']],
...
var groups={'geochang-prugio-34':[['주방',[2,3]],['거실',[0,1]],['욕실',[4,5]],['현관·침실',[6,7,8,9]]],'prugio-castle-a-32':[['현관',[10]],['거실·주방',[0,1,2,3,4,5]],['욕실',[6,7]],['침실·드레스룸',[8,9]]],'bonggok-hyunjin-36':[['거실·주방',[0,1,2]],['욕실',[3,4,9]],['현관·침실',[5,6,7,8]]],'imeun-kolon-35':[['거실·주방',[0,1,2]],['욕실',[3,4]],['현관·침실·수납',[5,6,7,8,9]]]};
var descriptions=spaces[row.slug]||[], beforeConfig={'geochang-prugio-34':['거창 푸르지오 34평','기존 공간의 구조와 마감 상태를 확인한 뒤 주방 동선과 수납 계획을 우선 정리했습니다.'],'prugio-castle-a-32':['구미 푸르지오캐슬 A단지 32평','기존 마감과 수납 상태를 확인한 뒤 공간 연결성과 생활 동선을 중심으로 정리했습니다.']}[row.slug], before=beforeConfig?'<section class="detail-section"><h2>BEFORE · 공사 전</h2><div class="detail-space-gallery">'+['before-01.jpg','before-02.jpg','before-03.jpg','before-04.jpg','before-05.jpg'].map(function(photo){return '<figure><img src="portfolio-assets/projects/'+row.slug+'/'+photo+'" alt="'+esc(beforeConfig[0])+' 공사 전 사진" loading="lazy"><figcaption>BEFORE · 공사 전 사진</figcaption></figure>';}).join('')+'</div><p>'+esc(beforeConfig[1])+'</p></section>':'';
var sections=(groups[row.slug]||[]).map(function(group){var item=descriptions.find(function(x){return group[0].indexOf(x[0])===0||x[0]===group[0];});var photos=group[1].map(function(index){var photo=row.photos[index];return photo?'<figure><img src="'+esc(photo)+'" alt="'+esc(row.title)+' '+esc(group[0])+' AFTER 사진" loading="lazy"><figcaption>AFTER · 완공 사진</figcaption></figure>':'';}).join('');return '<section class="detail-section"><h2>'+esc(group[0])+' · AFTER</h2><div class="detail-space-gallery">'+photos+'</div><p>'+esc(item?item[1]:'공간의 쓰임과 동선을 기준으로 마감과 수납을 정리했습니다.')+'</p></section>';}).join('');
root.innerHTML='<article class="detail shell"><a class="detail-back" href="portfolio.html">← 포트폴리오</a><header class="detail-head"><p class="detail-kicker">DAHAM PROJECT</p><h1>'+esc(row.title)+'</h1><p class="detail-meta">'+esc([row.location,row.area,row.kind].filter(Boolean).join(' · '))+'</p><p class="detail-summary">'+esc(desc)+'</p></header><div class="detail-rule"></div>'+before+sections+'<section class="detail-cta"><h2>비슷한 현장 상담이 필요하신가요?</h2><p>구미·김천·대구 인테리어 상담부터 현장 실측, 시공 후 A/S까지 직접 안내드립니다.</p><a href="./#footer">견적 문의하기</a></section></article>';
})();
