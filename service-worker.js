...
self.addEventListener('push',function(event){
  ...
  event.waitUntil(self.registration.showNotification(data.title||'다함 ERP',{
    ...
    tag:data.tag||undefined,renotify:!!data.renotify,data:{target:data.target||'erp.html'}
  }));
});

self.addEventListener('notificationclick',function(event){
  event.notification.close();
  var target=event.notification.data&&event.notification.data.target||'erp.html';
  var url=new URL(target,self.location.origin);
  if(url.origin!==self.location.origin)url=new URL('erp.html',self.location.origin);
